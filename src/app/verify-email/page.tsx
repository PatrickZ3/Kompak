...
export default function VerifyEmailPage() {
  ...
  async function verifyEmail() {
    try {
      // Reset to loading state if retrying
      setStatus("loading")

      // If no token is provided in the URL, update state with an error message
      if (!token) {
        setStatus("error")
            ...
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="flex flex-col space-y-2 pt-4 sm:flex-row sm:space-x-2 sm:space-y-0">
                {!token ? (
                  ...
                  <Button variant="outline" className="border-border text-secondary-foreground" onClick={verifyEmail}>
